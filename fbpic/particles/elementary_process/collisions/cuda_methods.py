...
def perform_collisions_cuda(N_batch, batch_size, npairs_tot,
    ...
    if i < N_batch:
        ...
        for ip in range(i*batch_size, N_max):
            ...
            U2 = xoroshiro128p_uniform_float64(
                random_states, i)    # random float [0,1]
            if U2 * w2[si2] < w1[si1]:
...
@cuda.jit
def alt_perform_collisions_cuda(N_batch, batch_size, npairs_tot,
                            prefix_sum1, prefix_sum2,
                            shuffled_idx1, shuffled_idx2, cell_idx,
                            n1, n2, T1, T2,
                            n12, m1, m2,
    ...
    if i < N_batch:
        ...
        for ip in range(i*batch_size, N_max):
            ...
            inv_gamma12 = 1. / gamma12

            # Center of mass (COM) velocity
            COM_vx = (px1 + px2) * inv_gamma12
            COM_vy = (py1 + py2) * inv_gamma12
            COM_vz = (pz1 + pz2) * inv_gamma12

            COM_v_v1 = (COM_vx * vx1 + COM_vy *
                        vy1 + COM_vz * vz1) / gamma1
            COM_v_v2 = (COM_vx * vx2 + COM_vy *
                        vy2 + COM_vz * vz2) / gamma2

            COM_v2 = COM_vx**2 + COM_vy**2 + COM_vz**2
            COM_gamma = 1. / m.sqrt(1. - COM_v2 / c**2)

            # momenta in COM
            term0 = ((COM_gamma - 1.) * COM_v_v1 / COM_v2 - COM_gamma) * m1 * gamma1
            px_COM = px1 + COM_vx * term0
            py_COM = py1 + COM_vy * term0
            pz_COM = pz1 + COM_vz * term0
            ...
            if logL <= 0.:
                coeff = 1. / (4. * m.pi * epsilon_0 * c**2)
                b0 = abs(coeff * q1 * q2 * COM_gamma * inv_gamma12 *
                    (m1 * gamma1_COM * m2 * gamma2_COM * invp_COM2 * c**2 + 1.))
                bmin = max(0.5 * h / p_COM, b0)
                if T1[cell] == 0 or T2[cell] == 0:
                    logL = 0
                else:
                    Debye2 = (epsilon_0 * k / e**2) / \
            ...
            coeff1 = 1. / (4. * m.pi * epsilon_0**2 * c**4)
            term1 = n1[cell] * n2[cell] * period * dt / n12[cell]
            term2 = coeff1 * (q1 * q2)**2 / (m1 * gamma1 * m2 * gamma2)
            term3 = COM_gamma * p_COM * inv_gamma12
            term4 = (m1 * gamma1_COM * m2 * gamma2_COM * invp_COM2 * c**2 + 1.)

            # Calculate the collision parameter s12
            s12 = logL * term1 * term2 * term3 * term4 * term4

            v_rel = p_COM * inv_gamma12 / ( COM_gamma * m1 * gamma1_COM * m2 * gamma2_COM )
            s_prime = (4. * m.pi / 3.)**(1/3) * term1 * \
                ((m1 + m2) / max(m1 * n1[cell]**(2/3), m2 * n2[cell]**(2/3))) * \
                v_rel
            ...
            vC_pf1COM = (COM_vx * pxf1_COM
                        + COM_vy * pyf1_COM
                        + COM_vz * pzf1_COM)
            vC_pf2COM = (COM_vx * pxf2_COM
                        + COM_vy * pyf2_COM
                        + COM_vz * pzf2_COM)
            ...
            if w2[si2] > U1 * max(w1[si1], w2[si2]):
                # Deflect particle 1
                term0 = (COM_gamma - 1.) * vC_pf1COM / COM_v2 + m1 * gamma1_COM * COM_gamma
                px1 = pxf1_COM + COM_vx * term0
                py1 = pyf1_COM + COM_vy * term0
                pz1 = pzf1_COM + COM_vz * term0
            ...
            U2 = xoroshiro128p_uniform_float64(
                random_states, i)    # random float [0,1]
            if w1[si1] > U2 * max(w1[si1], w2[si2]):
                # Deflect particle 2 (pf2 = -pf1)
                term0 = (COM_gamma - 1.) * vC_pf2COM / COM_v2 + m2 * gamma2_COM * COM_gamma
                px2 = pxf2_COM + COM_vx * term0
                py2 = pyf2_COM + COM_vy * term0
                pz2 = pzf2_COM + COM_vz * term0
