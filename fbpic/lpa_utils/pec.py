...
class PEC(object):
    ...
    def reflect_fields_at_pec( self, interp, comm, iteration ):
        ...
        if self.z_start != None:
            ...
            h_pec = 0.009579285714285714
            L_pec = 0.05747571428571428
            
            im = int( (self.z_start - zmin) / interp[0].dz)
            imax = int( (self.z_start+h_pec - zmin) / interp[0].dz)
            imin = int( (self.z_start-L_pec - zmin) / interp[0].dz)
            n_cells = 3
            ...
            for i, grid in enumerate(interp):
                ...
                Bz = getattr( grid, 'Bz')
                """
                Er[ im, :] -= 0.75*(self.Er_old[ im, :] \
                        - self.Er_old[ im, :]*(P00n( Z[ im, :], h_pec, L_pec )) \
                        - self.Er_old[ imax, :]*(P01n( Z[ im, :], h_pec,L_pec )))
                Et[ im, :] -= 0.75*(self.Et_old[ im, :] \
                        - self.Et_old[ im, :]*(P00n( Z[ im, :], h_pec, L_pec )) \
                        - self.Et_old[ imax, :]*(P01n( Z[ im, :], h_pec, L_pec )))      
                Ez[ im, :] -= 0.75*(self.Ez_old[ im, :] \
                        - self.Ez_old[ imax, :]*(P01n( Z[ im, :], h_pec, L_pec )))
                Er[ im, -1] = 0.
                Et[ im, -1] = 0.
                Ez[ im, -1] = 0.
                Er[ im, 0] = 0.
                ...
                if iteration % 500 == 0:
                    ...
                    self.complexDTVFilter(Bz, 9, 1, z, r, self.segments, self.normal)
                    self.complexDTVFilter(Br, 9, 1, z, r, self.segments, self.normal)
                    self.complexDTVFilter(Bt, 9, 1, z, r, self.segments, self.normal)
